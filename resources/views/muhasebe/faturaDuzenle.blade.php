...
{{Form::model($fatura,array('route' => ['satislar.faturaduzenle',$fatura->id] , 'method' => 'post'))}}
<script>
</script>
<div class="row">
    <div class="col-sm-12 portlets">
        <div class="widget">
            ...
            <div class="widget-content padding">
                ...
                <div class="form-group">
                    <label>Müşteri</label>
                    <input id="customers" type="text" class="form-control" placeholder="Müşteri adını yazınız" autocomplete="off"  name="customers" value="{{$customer->name}}"/>
                    {{Form::text('customers_id',null,array('class' => 'form-control','id'=>'customers_id' ,'hidden','onchange' => 'tamamlaMusteri()'))}}
                ...
                <div class="form-group">
                    ...
                    {{Form::date('expiryDate',null,array('class' => 'form-control'))}}
                </div>
                <div class="form-group">
                    <div class="col-sm-12">
                ...
                <div class="form-group">
                    <label>Sıra No</label>
                    {{Form::number('rowNumber',null,array('class' => 'form-control'))}}
                </div>
            ...
            <div class="md-modal md-rotate-from-bottom" id="md-rotate-from-bottom" >
                <div class="md-content"style="background-color: #E4EAE6">
                    <div>
                        ...
                        {{Form::select('currency',array('TRL' => 'Türk Lirası', 'USD' => 'Amerikan Doları','EUR' => 'Euro','GBP'=>'İngiliz Sterlini'),null,array('class' => 'form-control','id'=>'currency'))}}
                        <label>Döviz Kuru</label>
                        {{Form::number('exchangeRate',null,array('class' => 'form-control','step'=>'0.01','id'=>'exchangeRate','onchange'=>'hesapla()'))}}
                        <br>
            ...
            <div class="widget-content">
                <div class="table-responsive">
                    <table id="tablocuk" data-sortable class="table">
                        ...
                        <tr>
                            <td><input name="urun[{{$sayac}}][urun]" id="urun" type="text" class="form-control urunler{{$sayac}}" placeholder="Urun adını yazınız" autocomplete="off" value="{{\App\Models\Back\Stock\products::find($content->products_id)->name}}"  />

                                {{Form::text('urun['.$sayac.'][urunID]',$content->products_id,array('class' => 'form-control urunID'.$sayac,'hidden','style'=>'display:auto','id'=>'urunID'))}}
                            </td>
                            <td>{{Form::number('urun['.$sayac.'][miktar]',$content->stock,array('class' => 'form-control miktar'.$sayac,'id'=>'miktar','oninput'=>'hesapla(1)'))}}</td>
                            <td>{{Form::number('urun['.$sayac.'][birimFiyat]',$content->unit_price,array('class' => 'form-control birimFiyat'.$sayac,'id'=>'birimFiyat','onchange'=>'hesapla(1)','step'=>'0.01'))}}</td>
                            <td>{{Form::select('urun['.$sayac.'][vergi]',array('0' => 'KDV %0', '1' => 'KDV %1' , '8' => 'KDV %8', '18' => 'KDV %18'),$content->tax,array('class' => 'form-control vergi'.$sayac,'id'=>'vergi','onchange'=>'hesapla(1)'))}}</td>
                            <td>{{Form::number('urun['.$sayac.'][toplam]',($content->stock)*($content->unit_price),array('class' => 'form-control toplam'.$sayac,'id'=>'toplam','step'=>'0.01'))}}</td>
                        ...
                        <tr>
                            <td id="birim2" class="birimler{{$sayac}}" style="display: none">{{Form::number('urun['.$sayac.'][birim]',null,array('class' => 'form-control','id'=>'birim' ))}}</td>
                            <td id="aciklama2" class="aciklamalar{{$sayac}}" style="display: none">{{Form::text('urun['.$sayac.'][aciklamaIcerik]',null,array('class' => 'form-control','style', 'id'=>'aciklamaIcerik'))}}</td>
                            <td id="indirim2" class="indirimler{{$sayac}}" style="display: none">{{Form::text('urun['.$sayac.'][indirim]',null,array('class' => 'form-control','style','id'=>'indirim'))}}</td>
                            <td id="otv2" class="otvler{{$sayac}}" style="display: none">{{Form::text('urun['.$sayac.'][otv]',null,array('class' => 'form-control','style','id'=>'otv'))}}</td>
                            <td id="oiv2" class="oivler{{$sayac}}" style="display: none">{{Form::text('urun['.$sayac.'][oiv]',null,array('class' => 'form-control','style','id'=>'oiv'))}}</td>
                        </tr>
                        @php $sayac++ @endphp

                        @endforeach
                        {{Form::text('sayac',$sayac,array('hidden','id'=>'sayac'))}}
                        </tbody>
                    </table>
                    <div class="form-group">
                        <div class="col-sm-3">
